...
const generateAvatarButton = document.getElementById('generate-avatar-button') as HTMLButtonElement;
...
const moduleData: Record<ModuleKey, ModuleDataConfig> = {
    ...
    'programming': {
        ...
        image: "url('https://images.unsplash.com/photo-1555066931-4365d14bab8c?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3wzOT二次A3fDB8MXxzZWFyY2h8N3x8Y29kaW5nfGVufDB8fHx8MTcxNjQxOTU3N3ww&ixlib=rb-4.0.3&q=80&w=1080')",
...
function initializeApp() {
    ...
    try {
        ...
        console.error(error);
        handleFatalError("Failed to initialize the AI service. Please check your API key.");
        return;
    }
    ...
    // Profile Actions
    generateAvatarButton?.addEventListener('click', handleGenerateAvatar);
...
function handleGoogleSignIn() {
    ...
    // In a real application, this would trigger the Google Sign-In OAuth flow.
    try {
        ...
        // Also add them to the general users list if they aren't there
        const users = JSON.parse(localStorage.getItem('users') || '[]');
        if (!users.some(u => u.email === mockGoogleUser.email)) {
            // Note: We don't add a password for OAuth users
            users.push({ email: mockGoogleUser.email, name: mockGoogleUser.name });
            localStorage.setItem('users', JSON.stringify(users));
        }

        // Use the existing function to transition to the app
        checkAuthStatus();
...
async function handleGenerateAvatar() {
    if (!generateAvatarButton || !avatarLoader || !profileAvatarImg) return;

    generateAvatarButton.disabled = true;
    avatarLoader.classList.add('active');

    try {
        const prompt = "A futuristic or cartoon-style profile portrait of a unique character, digital avatar, upper body, centered, highly stylized, vibrant colors, clean background, variety in gender, skin tone, and accessories (e.g., goggles, helmets, glowing eyes, sci-fi clothing, robotic parts), suitable for a profile picture, 1:1 aspect ratio";
        ...
        addMessageToHistory("Sorry, I couldn't generate a new avatar right now. Please try again later.", 'ai', true);
    } finally {
...
async function handleRunCode() {
    if (!codeInput || !codeOutput || !outputLoader || !runCodeButton) return;
